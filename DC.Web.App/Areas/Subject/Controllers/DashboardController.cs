...
namespace DC.Web.App.Areas.Subject.Controllers
{
    ...
    public class DashboardController : WFbaseController
    {
        ...
        public JsonResult Subject()
        {
            try
            {
                var results = new SubjectRepository().SelectList().ToList();
        ...
        public JsonResult Gets()
        {
            try
            {
                ...
                var datas =  repository.Gets().ToList();
        ...
        private SubjectModel GetData(long Id)
        {
            var model = new SubjectRepository().Get(Id);
            if (model != null)
            {
        ...
        public PartialViewResult Add(long Id, int PublisherId = 0, string Title="")
        {
            try
            {
                Title = string.IsNullOrWhiteSpace(Title) ? "Create New Subject" : Title;
                if (Id > 0)
                {
                ...
                {
                    return PartialView("ucAdd", new SubjectModel { Title = "",MasterPublisherId= PublisherId });
                }
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public ActionResult IsExist(long Id, string Name, long PublisherId)
        {
            var isExist = new SubjectRepository().IsExist(Id, Name );
            if (!isExist)
            {
                return Json(new
                {
        ...
        public ActionResult ActiveInactive(long Id)
        {
            try
            {
                if (Id > 0)
                {
                    var result = new SubjectRepository().ActiInactive(Id, User.Identity.GetUserId());
                    if (result != null)
                    {
                        return Json(new
                        {
        ...
        public ActionResult Delete(long Id)
        {
            try
            {
                if (Id > 0)
                {
                    var result = new SubjectRepository().InsertUpdateDelete(new SubjectModel { Id = Id, ActionTaken = DC.Action.Delete });
                    if (result != null)
                    {
                        return Json(new
                        {
                    ...
                    else
                    {
                        ...
                        {
                            StatusCode = 400,
                            Message = "Please check internet some error occur while deleting Subject.Please try again later."
                        }, JsonRequestBehavior.AllowGet);
                    }
                }
                else
